...
def imm_to_binary(value,no_of_bits):
    # handle already-resolved ints from label substitution and hex strings like 0x00000000
    ...
    for pc,instruction in parsed_dict.items():
        ...
        try:
            if op_type == "R": 
                ...
                raw = instruction[4]
                imm_value = (int(raw, 0) if isinstance(raw, str) else int(raw)) * 4  # multiplying relative instruction offset by 4 for byte addressing
                ...
                rd = REGISTERS[instruction[2]]
                imm_value = instruction[3]
                imm = imm_to_binary(imm_value,20)   # 20-bit field directly; old code used 32-bit then sliced wrong bits
                opcode = U_TYPE[op]
