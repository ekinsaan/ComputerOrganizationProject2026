...
def validate(parsed_output):
    ...
    #check for virtual halt (must exist AND be the last instruction)
    last_pc = max(parsed_output.keys())
    last = parsed_output[last_pc]
    def _is_vhalt(instr):
        if instr[1] != "beq" or instr[2] != "zero" or instr[3] != "zero":
            return False
        raw = instr[4]
        try:
            return (int(raw, 0) if isinstance(raw, str) else int(raw)) == 0
        except (ValueError, TypeError):
            return False

    halt_found = any(_is_vhalt(instr) for instr in parsed_output.values())
    if not halt_found:
        errors.append("Virtual halt is missing")
    elif not _is_vhalt(last):
        errors.append("Virtual halt must be the last instruction")
